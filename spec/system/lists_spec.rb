...
descrice '投稿のテスト' do
  ...
  descrice 'トップ画面(top_path)のテスト' do
   before do
     visit top_path
   end
     context '表示の確認' do
...
descrice '一覧画面のテスト' do
  before do
    visit todolists_path
  end
  context '一覧の表示とリンクの確認' do
    it '一覧表示画面に投稿されたもの表示がされているか' do
...
descrice '詳細画面のテスト' do
  ...
  context '表示のテスト' do
   ...
   it '編集リンクが存在しているか' do
     pect(page).to have_link '編集'
   end
  end
  context 'リンクの遷移先の確認' do
    it '編集の遷移先は編集画面か' do
      edit_link = find_all[0]
      edit_link.click
